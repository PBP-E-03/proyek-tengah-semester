document.addEventListener('DOMContentLoaded', async () => {
  ...
  const showRegions = async () => {
    ...
    const response = await fetch(`/location/${code}`);
  ...
  closeAlertS.addEventListener('click', async (event) => {
    $('#full-success-alert').delay(100).fadeOut();
  });
  closeAlertE.addEventListener('click', async (event) => {
    $('#full-error-alert').delay(100).fadeOut();
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const formData = {
      ...
      donate_for_someone: document.getElementById('id_donate_for_someone')
        .checked,
    ...
    ).innerText = `You just donated ${formData['amount']} trees in ${
      formData['region']
    }.
    ...
    document.getElementById(
      'error-alert-message'
    ).innerText = `Please check the fields and make sure you have filled everything required correctly`;
    $('html, body').animate({ scrollTop: 0 }, 'fast');

    $fullAlert.delay(100).fadeIn().delay(3000).fadeOut();
